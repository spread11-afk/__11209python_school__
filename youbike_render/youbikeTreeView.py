...
class ShowDetail(Dialog):
    ...
    def body(self, master):
        ...
        snaVar = tk.StringVar()
        snaVar.set(self.sna)
        tk.Entry(mainFrame, textvariable=snaVar,
                 state='disabled').grid(column=1, row=0)

        mdayVar = tk.StringVar()
        mdayVar.set(self.mday)
        tk.Entry(mainFrame, textvariable=mdayVar,
                 state='disabled').grid(column=1, row=1)

        sareaVar = tk.StringVar()
        sareaVar.set(self.sarea)
        tk.Entry(mainFrame, textvariable=sareaVar,
                 state='disabled').grid(column=1, row=2)

        arVar = tk.StringVar()
        arVar.set(self.ar)
        tk.Entry(mainFrame, textvariable=arVar,
                 state='disabled').grid(column=1, row=3)

        totVar = tk.StringVar()
        totVar.set(self.tot)
        tk.Entry(mainFrame, textvariable=totVar,
                 state='disabled').grid(column=1, row=4)

        sbiVar = tk.StringVar()
        sbiVar.set(self.sbi)
        tk.Entry(mainFrame, textvariable=sbiVar,
                 state='disabled').grid(column=1, row=5)

        bempVar = tk.StringVar()
        bempVar.set(self.bemp)
        tk.Entry(mainFrame, textvariable=bempVar,
                 state='disabled').grid(column=1, row=6)
    ...
    def buttonbox(self):
        ...
        w = tk.Button(box, text="確認", width=10,
